...
const FacebookConnectionModal = ({ isOpen, onClose, onConnect, userId }) => {
  ...
  const handleStartAuth = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8001/api/auth/facebook/initiate', {
        method: 'POST',
  ...
  const handleFinishConnection = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8001/api/auth/facebook/save-connection', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          user_id: userId,
